					...
					<el-col :span="12">
						<el-form-item label="负责人">
							<div class="d-flex align-items-center">
								<el-select v-model="projectForm.dept_id" clearable filterable placeholder="请选择负责人所在部门" class="w-100" @change="deptChange2">
									<el-option
										v-for="item in deptOptions"
										:key="item.wid"
										:label="item.name"
...
<script>
	...
	export default {
        ...
        rules: {
					p_dept:[
            { required: true, message: '请选择可用部门', trigger: 'change' }
          ],
					apply_id:[
		...
		methods:{
			...
			applyChange(val){
				let obj = {};
				obj = this.userOptions.find((item)=>{//这里的userOptions就是上面遍历的数据源
