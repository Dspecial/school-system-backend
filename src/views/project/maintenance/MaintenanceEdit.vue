			...
			<el-form :model="maintenanceForm" :rules="rules" ref="maintenanceForm" label-width="110px" label-position="top" class="pl-3 pr-3">
                ...
                value-format="yyyy"
					...
					<el-col :span="24" v-if="maintenanceForm.money > 0">
						<el-form-item label="合同付款信息" class="payment_item">
	...
	export default {
		...
		methods:{
			...
			openEdit(){
				...
				this.$api.p_maintenance_edit({
					...
					// 合同付款节点
					this.agree_payinfo = data.data.pay_info;

					// 合同附件
					this.filesList = data.data.info.files_arr;

					// 项目申请编号，用到上传付款信息时候的接口上
					this.maintenanceForm.apply_number = data.data.project_info.apply_number;
				})
			},
      ...
      // form提交
			submitForm(formName) {
				...
				var ispayArr = this.commonJs.isEmpty(this.agree_payinfo[0]);
				if(!ispayArr){
					payArr = this.agree_payinfo.map((item)=>{
						...
						return {
							...
							haspaytime: item.haspaytime?item.haspaytime:"",
							is_pay: item.is_pay,
							remark: item.remark,
        ...
        files = this.filesList.map((item)=>{
	      ...
	      formData.append("apply_number", this.maintenanceForm.apply_number + '/' + this.maintenanceForm.extend_number);
      ...
      beforeUpload_pay(file,cell) {
	      ...
	      formData.append("user_token", this.$cookies.get("application_token"));
				...
				this.$api.p_maintenance_upload(formData).then(data =>{
					if(data.code == 0){
						// 回调成功的方法
						params.onSuccess(data);
						this.$message.success(data.msg);
      ...
      handleRemove(file,fileList) {
      	...
      	if(file.isExist){ // 原先上传已存在的
			...
			// 删除调接口
			removeFile(path){
				this.$api.p_maintenance_uploadDel({
      		path:path,
      	}).then(data =>{
					if(data.code == 0){
